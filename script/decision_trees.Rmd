...
[map](https://www.kaggle.io/svf/441409/8c6ea1a2aa8cec052ca7959d43ad34a0/__results___files/figure-html/unnamed-chunk-21-1.png)
...
```{r}
library(class)
prescribers_pred<-knn(train=prescribers_n_train, test=prescribers_n_test, cl=prescribers_n_train_labels, k=140)
```


# Evaluating Model Performance

![](https://www.researchgate.net/profile/Willard_Freeman/publication/43072205/figure/fig1/AS:279029774077960@1443537290471/Figure-1-Metrics-for-assessing-biomarker-performance-Unlike-traditional-statistics-that.png)
...
![](C:\Users\PhamX\Courses\Fall 2016\BIA 6301\Week 2\Splitting_Criterion.jpg)
...
CIPROFLOXACIN.HCL: used to treat a variety of bacterial infection.  
LEVOTHROXINE.SODIUM: used to treat underactive thyroid (hypothyrodism).  
MELOXICAM: nonsteroid anti-inflammatory drug.  
LYRICA: used to treat pain caused by nerve damage due to diabetes, shingles, spinal cord injury, and fibromyalgia; seizures  
...
```{r}
prescribers_train_logit<-prescribers_train[,c(1,44,126,137,139,298,328)]

prescribers_train_logit$LEVOTHYROXINE.SODIUM.recode<-ifelse(prescribers_train_logit$LEVOTHYROXINE.SODIUM<22,"no","yes")

prescribers_train_logit$CIPROFLOXACIN.HCL.recode<-ifelse(prescribers_train_logit$CIPROFLOXACIN.HCL<6,"no","yes")

prescribers_train_logit$MELOXICAM.recode<-ifelse(prescribers_train_logit$MELOXICAM<6,"no","yes")

prescribers_train_logit$LYRICA.recode<-ifelse(prescribers_train_logit$LYRICA<12,"no","yes")

prescribers_train_logit[,c(2:5)]<-NULL

names(prescribers_train_logit)

prescriber.logit <- glm(Opioid.Prescriber~., data=prescribers_train_logit, family=binomial()) #Fit a logistic regression
...
The odds of an Emergency Medicine (EM) doctor prescribing opioids is 32 times higher than their non-EM counterparts. 

The odds of a Surgeon prescribing opioids is 18 times higher than their non-Surgeon counterparts.

Doctors who prescribe CIPROFLOXACIN.HCL are four times more likely to prescribe opioids than doctors who do not prescribe CIPROFLOXACIN.HCL.

Doctors who prescribe LEVOTHROXINE.SODIUM are five times more likely to prescribe opioids than doctors who do not prescribe LEVOTHROXINE.SODIUM.
...
```{r}
prescribers_test_logit<-prescribers_test[,c(1,44,126,137,139,298,328)]

prescribers_test_logit$LEVOTHYROXINE.SODIUM.recode<-ifelse(prescribers_test_logit$LEVOTHYROXINE.SODIUM<22,"no","yes")

prescribers_test_logit$CIPROFLOXACIN.HCL.recode<-ifelse(prescribers_test_logit$CIPROFLOXACIN.HCL<6,"no","yes")

prescribers_test_logit$MELOXICAM.recode<-ifelse(prescribers_test_logit$MELOXICAM<6,"no","yes")

prescribers_test_logit$LYRICA.recode<-ifelse(prescribers_test_logit$LYRICA<12,"no","yes")

prescribers_test_logit[,c(2:5)]<-NULL
```
